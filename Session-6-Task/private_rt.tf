resource "aws_route_table" "webserver_private_rt" {
  ...
  route {
    ...
    gateway_id = aws_nat_gateway.webserver_nat_gw.id
  }

  tags = {
    Name    = "${var.env}-webserver-private-rt"
    Env     = var.env
...
resource "aws_route_table_association" "priv_sub_a" {
  ...
  route_table_id = aws_route_table.webserver_private_rt.id
